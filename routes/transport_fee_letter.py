...
bp = Blueprint('transport_fee_letter', __name__)
GENERATED_LETTERS_PATH = 'generated_letters'
os.makedirs(GENERATED_LETTERS_PATH, exist_ok=True)

@bp.route('/transport-fee-letter', methods=['POST'])
def transport_fee_letter():
        ...
        if total_due > 0:
            ...
            if student:
                ...
                letter_filename = create_fee_letter(
                    'templates/Transport.docx', student_name, parent_name, address, branch, roll_no,
...
def create_fee_letter(template_path, student_name, parent_name, address, branch, roll_no,
    ...
    filename = f"{GENERATED_LETTERS_PATH}/{roll_no}_transport_fee_letter_{current_date}.docx"
    doc.save(filename)
    return filename
...
def merge_pdfs(pdf_files):
    ...
    for pdf in pdf_files:
        merger.append(pdf)
    output_path = f"{GENERATED_LETTERS_PATH}/combined_transport_fee_letters_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf"
